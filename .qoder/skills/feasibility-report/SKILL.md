...
| 序号 | 任务名称 | 类型 | 状态 |
|------|---------|------|------|
| 1 | 创建文件 - 封面 + 目录 | [主流程] | [ ] 等待中 |
| 2 | 第1章 现状及必要性 | [Agent] | [ ] 等待中 |
| 3 | 第2章 建设目标及内容 | [Agent] | [ ] 等待中 |
| 4 | 第3章 技术方案 | [Agent] | [ ] 等待中 |
| 5 | 第4章 投资估算 | [Agent] | [ ] 等待中 |
| 6 | 第5章 项目组织及计划安排 | [Agent] | [ ] 等待中 |
| 7 | 第6章 可行性分析 | [Agent] | [ ] 等待中 |
```
...
**任务1（主流程直接写）：** 用 Write 创建文件，写入封面和目录。
...
所有章节生成完毕后，调用 docx 技能将 Markdown 文件转换为格式规范的 Word 文档：
...
1. 现状及必要性
   ...
   1.3 系统现状（仅提升推广类，其他类型跳过）
   1.4 主流产品分析（仅引进实施类，其他类型跳过）
   1.5 需求和必要性分析
2. 建设目标及内容
   2.1 顶层设计及总体规划
   2.2 项目建设目标
...
| 章节 | 适用条件 | 不适用时处理 |
|------|---------|------------|
| 1.3 系统现状 | 仅提升推广类 | 直接跳过，不生成该章节 |
| 1.4 主流产品分析 | 仅引进实施类 | 直接跳过，不生成该章节 |
...
- 每次只问一个问题，优先给选项
- 按项目类型裁剪章节：1.3 仅提升推广类生成，1.4 仅引进实施类生成，其他类型直接跳过这两节
- 编制说明内容不出现在最终报告中
- 数据不足时用合理的行业参考值填写（如并发用户数写"不低于100人"），不写"待定"；只有投资金额等真正无法估算的数字才填"待定"
