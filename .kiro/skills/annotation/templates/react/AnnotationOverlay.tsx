...
const DRAWER_VISIBLE_SELECTORS = ['.ant-drawer-open .ant-drawer-content-wrapper', '.el-drawer.is-visible', '.van-popup--left, .van-popup--right', '.t-drawer.t-is-visible', '.n-drawer-body-content']
...
export function AnnotationOverlay() {
  ...
  useEffect(() => {
    ...
    const mo = new MutationObserver(checkOverlayState)
  ...
  // FAB 拖动
  const handleFabDragStart = (e: React.MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      if (fabMovedRef.current) setFabPos({ right: Math.max(8, startRight - dx), bottom: Math.max(8, startBottom - dy) })
    }
    const onUp = () => { document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp); startFabIdleTimer() }
    ...
    <div className="annotation-overlay">
      ...
      {editMode && (
        <div className="annotation-toolbar">
          ...
          <button className="toolbar-btn" onClick={exportAnnotations}>导出</button>
