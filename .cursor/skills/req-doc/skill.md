...
**代码质量评估：** analyzer 完成后，检查摘要中的字段完整度和业务规则数量。若某模块摘要中：
- 字段数 < 3，且无业务规则 → 标记为"代码信息不足"
- 枚举值全部缺失，且无提示文案 → 标记为"代码信息不足"
...
| PRD 语言规范 | Read(".claude/knowledge/phase1-requirements/prd-language.md") | 禁止词汇、替换表 |
| 章节格式规范 | Read(".claude/knowledge/phase1-requirements/section-format.md") | 详细设计章节结构 |
| 质量审查清单 | Read(".claude/knowledge/phase1-requirements/srs-quality-checklist.md") | SRS四维度审查标准 |
| 模块依赖规范 | Read(".claude/knowledge/phase1-requirements/module-dependency.md") | 依赖关系描述方式 |
